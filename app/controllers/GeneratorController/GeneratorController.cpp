...
GeneratorController::GeneratorController(MainWindow* mw, Engine* engine, NotificationService* notify)
...
void GeneratorController::bind(QWidget* regexTab, QWidget* codeViewTab)
{
    ...
    if (auto minTab = mw_->findChild<QWidget*>(QString())) {
        ...
        if (btnGen) QObject::connect(btnGen, &QPushButton::clicked, this, &GeneratorController::generateCode);
    }
}
...
void GeneratorController::convert()
{
    ...
    if (!edit || !cmbNfa || !cmbDfa || !cmbMin) return;
    auto text = edit->toPlainText();
    auto rf = engine_->lexFile(text);
    auto parsed = engine_->parseFile(rf);
    if (parsed.tokens.isEmpty()) { notify_->warning("未找到Token定义"); return; }
    mw_->setParsed(new ParsedFile(parsed));
    mw_->setRegexHash(mw_->computeRegexHashPublic(text));
    mw_->clearPaths();
    cmbNfa->blockSignals(true); cmbNfa->clear(); cmbNfa->addItem("全部");
    for (const auto& t : parsed.tokens) cmbNfa->addItem(t.rule.name);
    cmbNfa->blockSignals(false); cmbNfa->setCurrentIndex(0);
    cmbDfa->blockSignals(true); cmbDfa->clear(); cmbDfa->addItem("全部");
    for (const auto& t : parsed.tokens) cmbDfa->addItem(t.rule.name);
    cmbDfa->blockSignals(false); cmbDfa->setCurrentIndex(0);
    cmbMin->blockSignals(true); cmbMin->clear(); cmbMin->addItem("全部");
    for (const auto& t : parsed.tokens) cmbMin->addItem(t.rule.name);
    cmbMin->blockSignals(false); cmbMin->setCurrentIndex(0);
    if (mw_->statusBar()) mw_->statusBar()->showMessage("转换成功"); notify_->info("转换成功");
}

void GeneratorController::generateCode()
{
    if (!mw_->getParsed()) { notify_->warning("请先转换"); return; }
    QVector<int> codes; auto mdfas = engine_->buildAllMinDFA(*mw_->getParsed(), codes);
    auto s = CodeGenerator::generateCombined(mdfas, codes, mw_->getParsed()->alpha);
    auto codeView = codeTab_->findChild<QPlainTextEdit*>("txtGeneratedCode"); if (codeView) codeView->setPlainText(s);
    QString base = mw_->ensureGenDirPublic(); QString ts = QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss");
    QString curHash = mw_->computeRegexHashPublic(regexTab_->findChild<QTextEdit*>("txtInputRegex")->toPlainText());
...
{
    if (!mw_->getParsed()) {
        auto edit = regexTab_->findChild<QTextEdit*>("txtInputRegex"); if (!edit) { notify_->warning("未找到正则输入控件"); return; }
        auto rf = engine_->lexFile(edit->toPlainText()); auto parsed = engine_->parseFile(rf);
        if (parsed.tokens.isEmpty()) { notify_->warning("未找到Token定义"); return; }
        mw_->setParsed(new ParsedFile(parsed));
    }
    QVector<int> codes; auto mdfas = engine_->buildAllMinDFA(*mw_->getParsed(), codes);
    auto outPane = mw_->findChild<QPlainTextEdit*>("txtLexResult"); auto srcPane = mw_->findChild<QPlainTextEdit*>("txtSourceTiny");
    QString src = srcPane ? srcPane->toPlainText() : QString(); if (src.trimmed().isEmpty()) src = QStringLiteral("abc123 def456");
    auto output = engine_->runMultiple(mdfas, codes, src);
    if (outPane) outPane->setPlainText(output);
