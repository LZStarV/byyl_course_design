...
void TestValidationController::bind(QWidget* testPage)
{
    ...
    if (btnPick)  QObject::connect(btnPick,  &QPushButton::clicked, mw_, &MainWindow::pickSample);
    if (btnRun)   QObject::connect(btnRun,   &QPushButton::clicked, mw_, &MainWindow::runLexer);
    if (btnSaveAs)QObject::connect(btnSaveAs,&QPushButton::clicked, mw_, &MainWindow::saveLexAs);
}
