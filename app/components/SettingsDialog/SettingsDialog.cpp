...
void SettingsDialog::buildUi()
{
    ...
    v->addLayout(lBtns);
    connect(btnAddRow, &QPushButton::clicked, [this]() { int r = tblTiers->rowCount(); tblTiers->insertRow(r); });
    ...
    connect(btnDefaults, &QPushButton::clicked, [this]() {
        auto v = QVector<Config::WeightTier>(); v.push_back({220, 3}); v.push_back({200, 4}); v.push_back({100, 1}); v.push_back({0, 0});
        edtOutDir->setText(QCoreApplication::applicationDirPath() + "/../../generated/lex");
        tblTiers->setRowCount(0);
        for (int i = 0; i < v.size(); ++i) { tblTiers->insertRow(i); tblTiers->setItem(i, 0, new QTableWidgetItem(QString::number(v[i].minCode))); tblTiers->setItem(i, 1, new QTableWidgetItem(QString::number(v[i].weight))); }
        chkSkipBrace->setChecked(false); chkSkipLine->setChecked(false); chkSkipBlock->setChecked(false); chkSkipHash->setChecked(false);
        chkSkipSingle->setChecked(false); chkSkipDouble->setChecked(false); chkSkipTemplate->setChecked(false);
    });
    connect(btnSave, &QPushButton::clicked, [this]() {
        if (!collectAndApply()) return;
        auto path = decideSavePath();
        if (path.isEmpty()) { QMessageBox::warning(this, QStringLiteral("保存失败"), QStringLiteral("无法确定保存路径")); return; }
        if (!Config::saveJson(path)) { QMessageBox::warning(this, QStringLiteral("保存失败"), QStringLiteral("文件写入失败")); return; }
...
void SettingsDialog::loadCurrent()
{
    ...
    tblTiers->setRowCount(0);
    QVector<int> probe; for (int i = 0; i <= 3; ++i) probe.push_back(i ? i * 100 : 0);
    QSet<int> emitted; for (int c : probe) { int w = Config::weightForCode(c); if (!emitted.contains(w)) emitted.insert(w); }
    ...
    for (int i = 0; i < tiers.size(); ++i) { tblTiers->setItem(i, 0, new QTableWidgetItem(QString::number(tiers[i].minCode))); tblTiers->setItem(i, 1, new QTableWidgetItem(QString::number(tiers[i].weight))); }
    chkSkipBrace->setChecked(Config::skipBraceComment()); chkSkipLine->setChecked(Config::skipLineComment()); chkSkipBlock->setChecked(Config::skipBlockComment()); chkSkipHash->setChecked(Config::skipHashComment());
    chkSkipSingle->setChecked(Config::skipSingleQuoteString()); chkSkipDouble->setChecked(Config::skipDoubleQuoteString()); chkSkipTemplate->setChecked(Config::skipTemplateString());
}

static bool parseInt(const QString& s, int& out)
{
    bool ok = false; out = s.trimmed().toInt(&ok); return ok;
}

bool SettingsDialog::collectAndApply()
{
    QString dir = edtOutDir->text().trimmed(); if (dir.isEmpty()) return false;
    QDir d(dir); if (!d.exists()) { if (!QDir().mkpath(dir)) return false; }
    QVector<Config::WeightTier> tiers;
    for (int r = 0; r < tblTiers->rowCount(); ++r) {
        auto mi = tblTiers->item(r, 0); auto wi = tblTiers->item(r, 1); if (!mi || !wi) continue;
        int m = 0, w = 0; if (!parseInt(mi->text(), m)) return false; if (!parseInt(wi->text(), w)) return false; tiers.push_back({m, w});
    }
    if (tiers.isEmpty()) return false;
    std::sort(tiers.begin(), tiers.end(), [](const Config::WeightTier& a, const Config::WeightTier& b){return a.minCode>b.minCode;});
    Config::setGeneratedOutputDir(dir);
    Config::setWeightTiers(tiers);
    Config::setSkipBrace(chkSkipBrace->isChecked()); Config::setSkipLine(chkSkipLine->isChecked()); Config::setSkipBlock(chkSkipBlock->isChecked()); Config::setSkipHash(chkSkipHash->isChecked());
    Config::setSkipSingle(chkSkipSingle->isChecked()); Config::setSkipDouble(chkSkipDouble->isChecked()); Config::setSkipTemplate(chkSkipTemplate->isChecked());
    return true;
}
