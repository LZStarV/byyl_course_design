...
QVector<MinDFA> Engine::buildAllMinDFA(const ParsedFile& pf, QVector<int>& codes){ QVector<MinDFA> result; codes.clear(); for(const auto& pt : pf.tokens){ if(pt.rule.isGroup){ QVector<ASTNode*> alts; collectAlternatives(pt.ast, alts); int base = pt.rule.code; int idx=0; bool tinyKw = pt.rule.name.startsWith("_keywords", Qt::CaseInsensitive) && (pt.rule.expr.contains("repeat", Qt::CaseInsensitive) || pt.rule.expr.contains("until", Qt::CaseInsensitive)); if(tinyKw){ QStringList kws({"if","then","else","end","repeat","until","read","write"}); for(auto kw : kws){ auto ast = makeKeywordCI(kw); auto nfa = buildNFA(ast, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(base + (idx++)); } } else { for(auto alt : alts){ auto nfa = buildNFA(alt, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(base + (idx++)); } } } else { auto nfa = buildNFA(pt.ast, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(pt.rule.code); } } return result; }
